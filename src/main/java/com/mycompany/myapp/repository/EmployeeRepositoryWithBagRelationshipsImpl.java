...
public class EmployeeRepositoryWithBagRelationshipsImpl implements EmployeeRepositoryWithBagRelationships {
    ...
    public Optional<Employee> fetchBagRelationships(Optional<Employee> employee) {
        return employee.map(this::fetchVehicles);
    }
    ...
    public List<Employee> fetchBagRelationships(List<Employee> employees) {
        return Optional.of(employees).map(this::fetchVehicles).orElse(Collections.emptyList());
    }

    Employee fetchVehicles(Employee result) {
        return entityManager
            .createQuery(
                "select employee from Employee employee left join fetch employee.vehicles where employee is :employee",
                Employee.class
            )
    ...
    List<Employee> fetchVehicles(List<Employee> employees) {
        ...
        List<Employee> result = entityManager
            .createQuery(
                "select distinct employee from Employee employee left join fetch employee.vehicles where employee in :employees",
                Employee.class
            )
