...
import { EntityArrayResponseType, CashBookBalanceService } from '../service/cash-book-balance.service';
...
export class CashBookBalanceComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICashBookBalance): number => this.cashBookBalanceService.getCashBookBalanceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.cashBookBalances = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICashBookBalance[] | null): ICashBookBalance[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.cashBookBalanceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
